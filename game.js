...
// ================================================================
// NOUVELLE PAGE D'ACCUEIL — JavaScript pour les onglets,
// médailles, tables à débloquer et parc dans l'onglet
// ================================================================
...
// ---- Médailles (10 niveaux) ----
const MEDAL_THRESHOLDS = [
    { name: 'Godzilla Bronze', icon: '🥉', req: 50 },
    { name: 'Godzilla Argent', icon: '🥈', req: 100 },
    { name: 'Godzilla Or', icon: '🥇', req: 200 },
    { name: 'Godzilla Platine', icon: '⭐', req: 350 },
    { name: 'Godzilla Burning', icon: '🔥', req: 500 },
    { name: 'Godzilla Cristal', icon: '💎', req: 700 },
    { name: 'Godzilla Éclair', icon: '⚡', req: 900 },
    { name: 'Godzilla Shine', icon: '🌟', req: 1100 },
    { name: 'Roi des Monstres', icon: '👑', req: 1400 },
    { name: 'Légende', icon: '🦖', req: 1800 }
];

const MEDAL_STORAGE_KEY = 'gmt_medals_v1';
...
function saveMedals(score) {
    try { localStorage.setItem(MEDAL_STORAGE_KEY, String(score)); } catch (e) {}
}

function updateMedals(score) {
    saveMedals(score);
    const grid = document.getElementById('medals-grid-start');
    if (!grid) return;
    grid.innerHTML = '';
    MEDAL_THRESHOLDS.forEach((medal, i) => {
        const unlocked = score >= medal.req;
        const div = document.createElement('div');
        div.className = 'medal-item' + (unlocked ? ' unlocked' : '');
        div.innerHTML = `
            <div class="medal-icon">${medal.icon}</div>
            <div class="medal-name">${medal.name}</div>
            <div class="medal-req">${unlocked ? '✅' : medal.req + ' pts'}</div>
        `;
...
// ---- Tables à débloquer ----
function renderTablesTrack() {
    const track = document.getElementById('tables-track-start');
    if (!track) return;
    track.innerHTML = '';
    const defeated = defeatedLevels || new Set();
    LEVELS.forEach((lvl, i) => {
        const chip = document.createElement('div');
        chip.className = 'table-chip';
        const tableNum = lvl.table;
        // Table 2 débloquée par défaut, les autres nécessitent la précédente
        const isUnlocked = i === 0 || defeated.has(i - 1);
        const isCompleted = defeated.has(i);
        if (isCompleted) {
            chip.classList.add('completed');
            chip.innerHTML = `<span class="table-number">×${tableNum}</span><span class="table-check">✅</span>`;
        } else if (isUnlocked) {
            chip.classList.add('unlocked');
            chip.innerHTML = `<span class="table-number">×${tableNum}</span>`;
            chip.title = `Débloqué ! Affronte la table ${tableNum}`;
            chip.addEventListener('click', () => {
                if (currentLevelIndex !== i || !document.getElementById('start-screen').classList.contains('hidden')) {
                    // Rejouer ce niveau depuis l'accueil
                    playSpecificLevel(i);
                }
            });
        } else {
            ...
            chip.innerHTML = `<span class="table-number">×${tableNum}</span>🔒`;
            chip.title = `Débloque d'abord la table ${LEVELS[i-1].table}`;
        }
        track.appendChild(chip);
    });
}

// ---- Mini Parc dans l'onglet ----
function renderParkInTab() {
    const grid = document.getElementById('park-grid-start');
    if (!grid) return;
    grid.innerHTML = '';
    const CELL_SIZE = 80;
    LEVELS.forEach((lvl, i) => {
...
// ---- Surcharge de la fonction goToMenu() pour mettre à jour la nouvelle page ----
const originalGoToMenu = goToMenu;
goToMenu = function() {
    originalGoToMenu();
    // Met à jour les éléments de la nouvelle page d'accueil
    const score = totalTimeBonus || 0;
    updateMedals(score);
    renderTablesTrack();
    renderParkInTab();
    updateStartButtonLabel();
...
// ---- Surcharge de startGame pour mettre à jour les médailles ----
const originalStartGame = startGame;
startGame = function() {
    originalStartGame();
    // on mettra à jour les médailles à la fin de la partie
};

// ---- Surcharge de levelUp pour mettre à jour les médailles ----
const originalLevelUp = levelUp;
levelUp = function(bonus) {
    originalLevelUp(bonus);
    const score = totalTimeBonus || 0;
    updateMedals(score);
    saveMedals(score);
};
